...
export const DistroBlueprintForm: React.FC<DistroBlueprintFormProps> = ({ config, onConfigChange, isLocked }) => {
    ...
    const handleArrayChange = (name: 'aurHelpers' | 'extraRepositories', value: string) => {
    ...
    return (
        <div className="p-5">
            ...
            <DetailSelect 
                ...
                tooltip={AI_RESOURCE_PROFILES[config.aiResourceAllocation]?.description}
                disabled={isLocked}
            />
             <DetailSelect 
                label="AI GPU Mode" 
            ...
            <DetailInput label="AUR Helper(s)" name="aurHelpers" value={config.aurHelpers.join(', ')} onChange={(e) => handleArrayChange('aurHelpers', e.target.value)} disabled={isLocked} />
            <DetailInput label="Extra Repos" name="extraRepositories" value={config.extraRepositories.join(', ')} onChange={(e) => handleArrayChange('extraRepositories', e.target.value)} disabled={isLocked} />
        </div>
    );
};